import { Container, Inject, Injectable } from '@rxdi/core';
import { ApolloClient } from '@rxdi/graphql-client';
import { MutationOptions, QueryOptions, SubscriptionOptions } from '@rxdi/graphql-client';
import { Component, html, property, TemplateObservable, unsafeHTML } from '@rxdi/lit-html';
...
import { IClientType, IClientViewType, IComponentsType, IMutation, ISubscription } from '../../../../../@introspection';
import { ClientViewRenderingEnum, ConditionalOperators } from '../../../../../shared/enums/render';
...
type QueryType = 'mutate' | 'query' | 'subscription' | 'subscribe';
...
interface IMixinReactToChanges {
...
export class ReactOnChangeService {
  ...
  private loadedComponents: Map<string, BehaviorSubject<IClientViewType>> = new Map();
  private routes = [];
  ...
  subscribeToAppChanges() {
    return merge(
      ...
      map(({ data: { clientReady, listenForChanges } }: IMixinReactToChanges) => clientReady || listenForChanges),
  ...
  private async loadComponents(change: IClientType) {
    await this.loadDynamicComponents(
      []
        .concat(...(change.components ? change.components : []), ...change.views.map(v => v.components))
        .filter(i => !!i),
    );
    return change;
  }
  ...
  private createExecutableQuery(type: GraphQLFieldMap<any, any>, method: GraphqlRequestTypes) {
  ...
  async applyChanges(change: IClientType) {
    ...
    const Queries = this.createExecutableQuery(schema.getQueryType().getFields(), GraphqlRequestTypes.query);
    const Mutations = this.createExecutableQuery(schema.getMutationType().getFields(), GraphqlRequestTypes.mutation);
    ...
    await Promise.all(
      change.views.map(async v => {
        ...
        const resolve = (path: string, obj: string, separator = '.') =>
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (Array.isArray(path) ? path : path.split(separator)).reduce((prev, curr) => prev && prev[curr], obj as any);
        const replaceSpecialCharacter = (
          ...
          let elementToRemove = [];
          const iterateOverElement = (elements: IElement[]) => {
            return elements.map(element => {
              if (element.children) {
                element.children = iterateOverElement(element.children);
              }
              if (element.attributes && element.type === 'element') {
                const specialConditionalOperator = element.attributes.find(a => a.key === ConditionalOperators['*if']);
                const specialIteratorOperator = element.attributes.find(a => a.key === ConditionalOperators['*let']);
                const ofOperator = element.attributes.find(a => a.key === ConditionalOperators.of);
                ...
                if (specialIteratorOperator && ofOperator) {
                  ...
                  const findRecursive = (childrens: IElement[]) => {
                    return childrens.map(c => {
                      if (c.type === 'text') {
                        const specialExtractor = c.content.match(new RegExp(options.left + '(.*)' + options.right));
                            ...
                            values
                              .map(val => {
                                return `<${element.tagName} ${element.attributes
                                  .map(a => ` ${a.key}="${a.value}"`)
                                  .join(' ')}>${resolve(key, val) || val}</${element.tagName}>`;
                              })
                              .join(' '),
                          );
                        }
                      ...
                      if (c.attributes) {
                        const TemplateOperator = c.attributes.find(a => a.key === ConditionalOperators['*template']);
                        if (TemplateOperator && c.children && c.children.length) {
                          const highOrderComponent = c.children.find(c => c.type === 'element');
                          ...
                          c.attributes.splice(c.attributes.indexOf(TemplateOperator), 1);
                        }
                      }
                      if (c.children) {
                ...
                if (specialConditionalOperator) {
                  const val = replaceSpecialBracklets(specialConditionalOperator.value);
              ...
              if (element.content) {
                const specialExtractor = element.content.match(new RegExp(options.left + '(.*)' + options.right));
                if (element.type === 'text' && specialExtractor && specialExtractor.length > 1) {
          ...
          const newTemplate = iterateOverElement(
            parse(template || '', {
              ...parseDefaults,
        ...
        class NewElement extends BaseComponent {
          ...
          private QueryTemplate = observable.pipe(
            ...
            tap(options => (this.options = options)),
            mergeMap(clientView =>
              combineLatest(of(clientView.html), clientView.query ? from(this.makeQuery()) : of({})).pipe(
                map(([html, query]) => replaceSpecialCharacter(html, query as string)),
                map(html => unsafeHTML(html)),
          ...
          private makeQuery() {
            ...
            if (isWrittenQuery) {
              const splittedQuery = this.options.query.split(' ') as QueryType[];
              queryType = splittedQuery[0];
              query = gql`
                ${this.options.query}
              `;
            } else {
            ...
            const options: QueryOptions | SubscriptionOptions | MutationOptions = {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return this[queryType as string](options).pipe(map(res => (res as { data: any }).data));
          }
