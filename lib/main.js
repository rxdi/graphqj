...
if (args_extractors_1.includes('--listen')) {
    let subscription;
    const path = args_extractors_1.nextOrDefault('--listen', './gj.ts');
    const ignored = path => path.includes('node_modules');
    chokidar_1.watch(path, { ignored }).on('change', (event, path) => __awaiter(this, void 0, void 0, function* () {
        if (subscription) {
    ...
    chokidar_1.watch(path, { ignored }).on('ready', (event, path) => __awaiter(this, void 0, void 0, function* () {
        if (subscription) {
            subscription.unsubscribe();
        }
        subscription = self_child_1.SelfChild().subscribe(process => {
