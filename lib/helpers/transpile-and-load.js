...
function TranspileAndLoad(path, outDir) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (core_1.Container.has(path)) {
            return core_1.Container.get(path);
        }
        yield typescript_builder_1.TranspileTypescript([path], outDir);
        const something = require(path_1.join(process.cwd(), outDir, path_1.parse(path_1.join(process.cwd(), outDir, path)).base.replace('ts', 'js')));
        core_1.Container.set(path, something);
        return core_1.Container.get(path);
    });
}
