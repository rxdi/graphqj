    ...
    Object.keys(config.$types).forEach(type => {
        ...
        Object.keys(currentType).forEach(key => {
            ...
            if (hasSymbol.length) {
                ...
                if (!usedExternalModule.module[methodToExecute]) {
                    throw new Error(`Missing method ${methodToExecute} inside ${usedExternalModule.file}`);
                }
                validators.push(usedExternalModule.module[methodToExecute]);
            ...
            types[type][key] = parse_types_schema_1.ParseTypesSchema(resolver, key, validators);
        });
